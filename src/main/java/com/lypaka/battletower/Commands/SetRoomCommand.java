...
public class SetRoomCommand extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(new TextComponentString("§cUso: /bt setroom <tier> <room>"));
            return;
        }
        ...
        try {
            ConfigGetters.setRoomLocation(tier, room, coord);
            sender.sendMessage(new TextComponentString("§aRoom " + room + " do Tier " + tier + " setado com sucesso!"));
        } catch (Exception e) {
